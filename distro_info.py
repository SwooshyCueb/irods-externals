...
try:
	...
	try:
		from distutuils.version import LooseVersion as DistroVersion
	except ImportError:
		...
		try:
			...
			logging.getLogger(__name__).warning('No disutils found, using packaging.version.Version as DistroVersion. If this results in errors, install the setuptools package.')
		except ImportError:
			logging.getLogger(__name__).error('Could not import version class. Please install the setuptools package.')
			raise
...
# For exmaple, CentOS and Rocky Linux will return `rhel`,
...
# Does not account for derivatives that deviate from the upstream
# version scheme (ex: RHEL7-like Amazon Linux 2).
...
def package_architecture_string():
	...
	log = logging.getLogger(__name__)
	if package_type() == 'deb':
		pkg_arch = subprocess.check_output(['dpkg', '--print-architecture'], timeout=5, encoding='locale')
